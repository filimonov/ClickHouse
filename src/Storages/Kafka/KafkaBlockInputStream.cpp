        ...
        if (buffer->poll())
        {
            ...
            catch (Exception & e)
            {
                if (put_error_to_stream)
                {
                    ...
                    result_columns = result_columns.cut();
                    for (size_t i = 0, s = columns.size(); i < s; ++i)
                    {
                        // read_kafka_message could already push some rows to result_columns
                        // before exception, we need to fix it.
                        if (result_columns[i].size() != total_rows)
                        {
                            // TODO: test it!
                            result_columns[i] = result_columns[i].cut(0,total_rows);
                        }
        ...
        if (new_rows)
        {
            ...
            for (size_t i = 0; i < new_rows; ++i)
            {
                ...
                if (put_error_to_stream)
                {
                    ...
                    {
                        virtual_columns[8]->insertDefault(); // raw_message
                        virtual_columns[9]->insertDefault(); // error
                    }
                }
            }
