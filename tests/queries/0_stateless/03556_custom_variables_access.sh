...
cleanup() {
    ...
    $CLICKHOUSE_CLIENT --query "DROP VARIABLE IF EXISTS access_cv"
}
trap cleanup EXIT
cleanup
...
$CLICKHOUSE_CLIENT --query "CREATE VARIABLE access_cv AS toUInt32(7)"

# --- CREATE_VARIABLE required for CREATE VARIABLE -------------------------
$CLICKHOUSE_CLIENT --user ${U_CREATE} --query "CREATE VARIABLE access_user AS 1" 2>&1 \
...
$CLICKHOUSE_CLIENT --user ${U_CREATE} --query "CREATE VARIABLE access_user AS toUInt32(1)" 2>&1 | head -1
$CLICKHOUSE_CLIENT --user ${U_CREATE} --query "DROP VARIABLE access_user" 2>&1 \
    | grep -q ACCESS_DENIED && echo "drop_without_grant=DENIED" || echo "drop_without_grant=ALLOWED"
$CLICKHOUSE_CLIENT --query "DROP VARIABLE access_user"

# --- DROP_VARIABLE required for DROP VARIABLE -----------------------------
$CLICKHOUSE_CLIENT --user ${U_DROP} --query "DROP VARIABLE access_cv" 2>&1 \
    | grep -q ACCESS_DENIED && echo "bare_drop=DENIED" || echo "bare_drop=ALLOWED"

# --- getVariable requires getVariable access ------------------------------
$CLICKHOUSE_CLIENT --user ${U_GET} --query "SELECT getVariable('access_cv')" 2>&1 \
...
$CLICKHOUSE_CLIENT --user ${U_GET} --query "SELECT getVariable('access_cv')"

# One getVariable grant covers all three read functions.
$CLICKHOUSE_CLIENT --user ${U_GET} --query "SELECT getVariableOrDefault('access_cv', toUInt32(0))"
$CLICKHOUSE_CLIENT --user ${U_GET} --query "SELECT getTemporaryVariableOrDefault('missing', toUInt32(0))"
$CLICKHOUSE_CLIENT --user ${U_GET} --query "SELECT getReplicatedVariableOrDefault('missing', toUInt32(0))"
...
$CLICKHOUSE_CLIENT --user ${U_GET} --query \
    "SELECT count() FROM system.custom_variables WHERE kind = 'server' AND name = 'access_cv'"

# --- SYSTEM REFRESH VARIABLE requires SYSTEM_CUSTOM_VARIABLES -------------
$CLICKHOUSE_CLIENT --user ${U_SYS} --query "SYSTEM REFRESH VARIABLE access_cv" 2>&1 \
...
$CLICKHOUSE_CLIENT --user ${U_SYS} --query "SYSTEM REFRESH VARIABLE access_cv" 2>&1 | head -1

# --- Readonly profile blocks CREATE VARIABLE ------------------------------
$CLICKHOUSE_CLIENT --query "GRANT CREATE_VARIABLE ON *.* TO ${U_RO}"
$CLICKHOUSE_CLIENT --user ${U_RO} --readonly 1 --query "CREATE VARIABLE access_ro AS 1" 2>&1 \
